...
function mcp1_recuperer_fond($flux){
	include_spip('inc/config');
	include_spip('inc/session');
	$fond=$flux['args']['fond'];
	...
	if($webmestre!='oui'){
		...
		$cacher_documents=lire_config('mcp1/cacher_documents')?lire_config('mcp1/cacher_documents'):array();
	...
	if($fond=='inclure/rubriques' AND lire_config('mcp1/menu_barre_nav')){	
		 $flux['texte']='';
		}
		//echo $fond;	
    return $flux;
